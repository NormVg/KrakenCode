...
## Rule: Auto-Commit
Whenever you (the AI agent) make meaningful changes to the codebase, automatically run `git add` and `git commit` with a descriptive message. Do NOT wait for user permission to commit your work.

## Rule: Project Documentation Tracking
Always refer to and maintain `AGENTS.md` for architectural guidelines, folder structures, and production-grade standards.
Always refer to and maintain `project.md` for project notes, current status, and long-term tasklists. Keep `project.md` updated as you complete tasks.
Always refer to the `.info2ai` folder for project documentation, library docs (like markstream), and extra context before writing code.
